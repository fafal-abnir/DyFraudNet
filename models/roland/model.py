...
class RolandGNN(torch.nn.Module):
    def __init__(self, input_dim, hidden_conv1, hidden_conv2, num_nodes, dropout=0.0, gnn_name="GCN", update='gru'):
        # """
        # Args:
...
class VahidGNN(torch.nn.Module):
    def __init__(self, input_dim, hidden_conv1, hidden_conv2, dropout=0.0, gnn_name="GCN", update='gru'):
        super().__init__()
        gnn_layer = gnn_layer_by_name[gnn_name]
        # TODO: I should find a solution for handling multiple layer forward
        self.hidden_conv1 = hidden_conv1
        ...
        # find better soluction for this part
        self.previous_embeddings = [
            torch.Tensor([[0 for _ in range(self.hidden_conv1)] for _ in range(self.hidden_conv1)]),
            torch.Tensor([[0 for _ in range(self.hidden_conv2)] for _ in range(self.hidden_conv2)])
        ]
