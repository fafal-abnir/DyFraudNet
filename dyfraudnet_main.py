...
def main():
    model = None
    dataset = DGraphFin('data/DGraphFin', force_reload=True,num_windows=30)
    for data_index in range(len(dataset) - 1):
        ...
        if model is None:
            model = DyFraudNet(snapshot.x.shape[1], memory_size=256, hidden_size=128, out_put_size=2, gnn_type="GCN")
